...
import { Search, MapPin, Sparkles, Loader2, X } from "lucide-react";
...
const Hero = ({
  ...
  const [activeIndex, setActiveIndex] = useState(-1); // Keyboard nav for direct match events

  // AI Suggestions
  const [aiSuggestions, setAiSuggestions] = useState([]);
  const [isAiLoading, setIsAiLoading] = useState(false);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [highlightedIndex, setHighlightedIndex] = useState(-1); // Keyboard nav for AI suggestions
  ...
  const fetchAiSuggestions = useCallback(async (q) => {
    ...
    try {
      const res = await fetch("/api/ai-search", {
        ...
        body: JSON.stringify({ query: q.trim(), location: locationCity }),
      });
      if (res.ok) {
  ...
  // Fetch autocomplete suggestions (DB events)
        ...
        >
          ...
          {(showDropdown || showSuggestions) && (
            ...
            >
                ...
                <div>
                  ...
                  <div
                    style={{
                      ...
                      padding: "8px 14px 6px",
                      borderBottom: "1px solid rgba(255,255,255,0.06)",
                      borderTop: showDropdown && suggestions.length > 0 ? "1px solid rgba(255,255,255,0.06)" : "none",
                    }}
                  >
                    {isAiLoading ? (
                      <>
              ...
              {/* No results state */}
              {showDropdown && !searchLoading && suggestions.length === 0 && !isAiLoading && aiSuggestions.length === 0 && searchQuery.trim() && (
                <div
                  style={{
                    padding: "20px",
                    textAlign: "center",
              ...
              {/* Footer hint */}
              {(((suggestions && suggestions.length > 0) && showDropdown) || ((aiSuggestions && aiSuggestions.length > 0) && showSuggestions)) && (
                <div
                  style={{
                    padding: "7px 14px",
                    fontSize: "10px",
            ...
            >
              {MOOD_PLACES[selectedMoodKey].map((place) => (
                <div
                  ...
                  onClick={() => {
                    if (isControlledSearch) {
                      externalSetSearchQuery(place.name);
                    } else {
                      setInternalSearchQuery(place.name);
                    }
                    if (onSearchChange) {
